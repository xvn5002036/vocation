import { AltarEntry, EarthlyBranch, FiveSystemEntry, HeavenlyStem, OriginPerson, TreasuryEntry } from './types.ts';

export const STEMS: HeavenlyStem[]=['甲','乙','丙','丁','戊','己','庚','辛','壬','癸'];
export const BRANCHES: EarthlyBranch[]=['子','丑','寅','卯','辰','巳','午','未','申','酉','戌','亥'];
export const GANZHI_LIST=Array.from({length:60},(_,i)=>`${STEMS[i%10]}${BRANCHES[i%12]}`);
const a=(altar:string,jing:string,governance:string):AltarEntry=>({altar:`${altar}壇`,jing:`${jing}靖`,governance});

/** 圖片第 5–7/51 頁：奏職雷壇法靖，依生年六十甲子查壇、靖、治炁。 */
export const YEAR_ALTAR_MAP:Record<string,AltarEntry>={
  甲子:a('靈真應妙','通玄致真','陽平治左平炁'),甲寅:a('神化通玄','體真會道','庚除治右都領炁'),甲辰:a('雷霆應化','靈虛自然','陽平治左平炁'),甲午:a('靈真應妙','成真集靈','北邙治右察炁'),甲申:a('神妙通玄','清真正性','蒙秦治左領功炁'),甲戌:a('雷霆應化','致真通玄','真多治右領神炁'),
  乙丑:a('靈應通真','復性澄真','真多治右領神炁'),乙卯:a('三界混玄','法天成真','鹿堂治右平炁'),乙巳:a('玉堂贊化','契真保元','太竹治右監察炁'),乙未:a('靈應通真','制魔澄真','主簿治右貢炁'),乙酉:a('三界混元','澄性成真','濃口治右監貢炁'),乙亥:a('玉堂贊化','降真集靈','北平治左監察炁'),
  丙子:a('飛捷報應','混玄致真','涌泉治左察神炁'),丙寅:a('應妙合英','通真澄化','庚除治右都領炁'),丙辰:a('靈一守玄','育神煉真','滴沅治右長炁'),丙午:a('飛捷報應','澄心得真','北邙治右察炁'),丙申:a('應妙合英','通真會應','公慕治右都炁'),丙戌:a('靈一守玄','保真湛然','蒙秦治左領炁'),
  丁丑:a('玄一靜玄','虛一通真','真多治右領神炁'),丁卯:a('三界集神','洞真自然','葛璝治左都領炁'),丁巳:a('正一妙玄','保真洞微','平蓋治右領功炁'),丁未:a('守一靜玄','無為契真','玉局治左察炁'),丁酉:a('三界集神','飛魔達真','濃口治右監貢炁'),丁亥:a('正一妙元','虛白通真','北平治左監察炁'),
  戊子:a('三台保護','自然合真','北邙治右察炁'),戊寅:a('玄真通妙','集真洞虛','秦中治左領神炁'),戊辰:a('三界混元','天一保真','滴沅治右長炁'),戊午:a('三台保護','純素沖真','鹿堂治右平炁'),戊申:a('玄真通妙','真元知化','公慕治右都炁'),戊戌:a('三界混真','存真守一','平岡治左貢炁'),
  己丑:a('道一守玄','虛一守真','昌利治左都監炁'),己卯:a('凝神定性','與道合真','葛璝治左都領炁'),己巳:a('三界集真','登心復真','平蓋治右領功炁'),己未:a('道一守玄','至一通真','主簿治右功炁'),己酉:a('凝神定性','皈真安然','昌利治左都監炁'),己亥:a('三界集真','念性守真','北平治左監察炁'),
  庚子:a('玄一守真','葆真混元','庚除治右都領炁'),庚寅:a('飛魔衍慶','澄真宏化','秦中治左領神炁'),庚辰:a('英化普照','超元洞真','鶴鳴治左長炁'),庚午:a('玄一守真','保性弘真','雲臺治右監貢炁'),庚申:a('飛魔衍慶','泰華澄真','公慕治右都炁'),庚戌:a('英化普照','玉虛洞真','雲臺治左監功炁'),
  辛丑:a('玄妙通真','輔元固真','赫上治右都監炁'),辛卯:a('三界集神','洞真自然','葛璝治左都領炁'),辛巳:a('坎離合元','涵虛復真','本竹治右監察炁'),辛未:a('玄妙通真','登真明性','玉局治左察炁'),辛酉:a('西觀淨清虛','太素頤真','後城治左都炁'),辛亥:a('坎離合玄','修真知性','本竹治右監察炁'),
  壬子:a('合妙混玄','通真洞清','桐柏治右監神炁'),壬寅:a('通玄會真','混合會真','秦中治左領神炁'),壬辰:a('璇璣運化','沖虛會真','鶴鳴治左長炁'),壬午:a('合妙混元','自然成真','桐柏治右監神炁'),壬申:a('通玄會真','混合明真','公慕治右都炁'),壬戌:a('璇璣運化','會元合真','庚除治右都領炁'),
  癸丑:a('百神集應','真光變化','赫上治右都監炁'),癸卯:a('玄妙應真','與聖合真','葛璝治左都領炁'),癸巳:a('玉堂靜玄','攝真召靈','平蓋治右領功炁'),癸未:a('百神集應','元超虛真','主簿治右貢炁'),癸酉:a('玄妙應真','清虛自然','後城治右都炁'),癸亥:a('玉堂靜玄','通真會元','涌泉治左監神炁')
};

/** 圖片第 7/51 頁：元命真人以生日地支論，原文並警示不可順便用年定。 */
export const ORIGIN_PERSON_MAP:Record<EarthlyBranch,OriginPerson>={
  子:{star:'貪狼星君',palace:'太上宮・天樞',surname:'徐'},丑:{star:'巨門星君',palace:'中元宮・天璇',surname:'張'},亥:{star:'巨門星君',palace:'中元宮・天璇',surname:'張'},
  寅:{star:'祿存星君',palace:'真元宮・天璣',surname:'葉'},戌:{star:'祿存星君',palace:'真元宮・天璣',surname:'葉'},卯:{star:'文曲星君',palace:'紐幽宮・天權',surname:'許'},酉:{star:'文曲星君',palace:'紐幽宮・天權',surname:'許'},
  辰:{star:'廉貞星君',palace:'綱神宮・玉衡',surname:'宋'},申:{star:'廉貞星君',palace:'綱神宮・玉衡',surname:'宋'},巳:{star:'武曲星君',palace:'紀明宮・開陽',surname:'蕭'},未:{star:'武曲星君',palace:'紀明宮・開陽',surname:'蕭'},午:{star:'破軍星君',palace:'開元宮・搖光',surname:'曹'}
};

/** 圖片第 6/51 頁：本命所屬寶庫，以生年天干論。 */
export const TREASURY_MAP:Record<HeavenlyStem,TreasuryEntry>={
  甲:{treasury:'御真瓊琳庫',office:'功德司',official:'陳玉'},乙:{treasury:'都盈瓊真庫',office:'照證司',official:'高明'},丙:{treasury:'金璧瓊臺庫',office:'平等司',official:'邪正'},丁:{treasury:'都攢瓊霄庫',office:'善惡司',official:'周全'},戊:{treasury:'金花庫',office:'拷掠司',official:'錢真'},己:{treasury:'玉籙瓊輪庫',office:'掠剩司',official:'石達'},庚:{treasury:'天元瓊品庫',office:'福德司',official:'左元'},辛:{treasury:'至聖瓊玉庫',office:'錄善司',official:'崔正'},壬:{treasury:'寶華瓊仙庫',office:'報應司',official:'吉兆'},癸:{treasury:'仙院瓊瑤庫',office:'超昇司',official:'甘上'}
};

export const ELEMENT_MARSHAL:Record<string,string>={木:'溫元帥',火:'馬元帥',土:'殷元帥',金:'趙元帥',水:'周元帥'};
export const FULL_MARSHAL_MAP:Record<string,string>={木:'地祇主令都巡太保溫元帥',火:'斗口靈官馬元帥',土:'地司太歲武光上將殷元帥',金:'上清正一龍虎執法趙元帥',水:'風輪蕩魔收怪滅邪周元帥'};
export const HOUR_VOCATION_MAP:Record<EarthlyBranch,string>={子:'鳳閣司籍仙官',丑:'彤華公案仙官',寅:'紫微校錄仙官',卯:'青城典籍仙官',辰:'天官校籍仙官',巳:'玉府執法仙官',午:'玄樞奏議仙官',未:'蓬萊定旨仙官',申:'紫府掌善仙官',酉:'太華司命仙官',戌:'集靈司功仙官',亥:'凝真紀錄仙官'};
export const HOUR_AUTHORITY_MAP:Record<EarthlyBranch,string>={子:'三界糾察便宜事',丑:'三界勘合便宜事',寅:'三界行移便宜事',卯:'三界點檢便宜事',辰:'三界推問便宜事',巳:'三界通傳便宜事',午:'三界執法便宜事',未:'三界理問便宜事',申:'三界便宜事',酉:'三界收管便宜事',戌:'三界點視便宜事',亥:'三界密察便宜事'};
export const QI_TITLE_MAP:Record<string,string>={木:'九炁君青天三五步罡',火:'三炁君赤天三五步罡',土:'一炁君黃天三五步罡',金:'七炁君白天三五步罡',水:'五炁君黑天三五步罡'};
export const MOUNTAIN_MAP:Record<string,string>={木:'東嶽',火:'南嶽',土:'中嶽',金:'西嶽',水:'北嶽'};
export const STEM_ELEMENT:Record<HeavenlyStem,string>={甲:'木',乙:'木',丙:'火',丁:'火',戊:'土',己:'土',庚:'金',辛:'金',壬:'水',癸:'水'};
export const GENERATING_ELEMENT:Record<string,string>={木:'水',火:'木',土:'火',金:'土',水:'金'};
export const BRANCH_MARSHAL:Record<EarthlyBranch,string>={子:'辛元帥',丑:'鄧元帥',寅:'趙元帥',卯:'張元帥',辰:'魯元帥',巳:'馬元帥',午:'王元帥',未:'殷元帥',申:'溫元帥',酉:'康元帥',戌:'成元帥',亥:'方元帥'};
export const SOLDIERS_MAP:Record<EarthlyBranch,string>={子:'一萬三千零五十名',丑:'七萬一千名',寅:'一萬一千名',卯:'三百八十名',辰:'一千四百七十名',巳:'七百名',午:'一千名',未:'十萬名',申:'一萬名',酉:'三萬名',戌:'八千名',亥:'五百名'};
export const FIVE_SYSTEM_MAP:Record<string,FiveSystemEntry>={
  木:{direction:'東嶽泰山・東華上相天尊',deity:'青帝九炁真人',phrases:['七真錫福','青帝垂恩','青篇紀名']},火:{direction:'南嶽衡山・南宮治煉天尊',deity:'赤帝三炁真人',phrases:['南丹治煉','赤帝垂恩','丹書紀善']},土:{direction:'中嶽嵩山・中黃帝一天尊',deity:'黃帝一炁真人',phrases:['元靈玄老','黃帝垂恩','玉格紀善']},金:{direction:'西嶽華山・西靈寶德天尊',deity:'白帝七炁真人',phrases:['西靈寶德','白帝垂恩','靈書紀善']},水:{direction:'北嶽恆山・北元總校天尊',deity:'黑帝五炁真人',phrases:['北源總校','黑帝垂恩','金書紀善']}
};
