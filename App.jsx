...
import { 
  Scroll, BookOpen, Menu, X, Info, ChevronDown, ChevronUp,
  Clock, Calendar, Shield, Activity, HelpCircle, MapPin, Star, UserCheck,
  Award, Landmark, FileText, Swords, Search, ShieldCheck
} from 'lucide-react';

// ==========================================
// 第一區塊：核心數據庫 (全量定義於頂層確保安全引用)
// ==========================================
...
const DAY_DUTIES = ["督理雲騎事務", "監察五方雷部", "統理陰陽律法", "巡察境內考較", "協理星府文書", "掌理符籙真氣", "宣化玄門正法", "鎮守法界樞機", "錄籍生靈功過", "保命長生護佑"];
...
const getMarshalIdxByOffset = (off) => {
...
const DICTIONARY_DATA = [
  {
    ...
    id: "admin",
    items: [
      { term: "太上三五都功職", meaning: "正一派法師的初階正式品秩。代表受領天師府法籙，具備通達天人的行政權限。" },
      { term: "宮 / 府 / 殿 / 院", meaning: "法師受職後的『靈魂辦公地址』。不同的宮署代表法師在天廷體系中不同的職能部門。" },
      { term: "知 [某某司] 兼 [某某事]", meaning: "明確法師的行政管轄權。『知』為正職，『兼』為監理職。這決定了法師行法時能處理的事務。" }
    ]
  },
  {
    category: "地理座標 (法壇定位)",
    id: "geo",
    items: [
      { term: "二十四治 (如：陽平治)", meaning: "源於張道陵祖師建立的行政區劃。法師名籍歸屬於特定的治所，類似於法界的戶籍。" },
      { term: "壇 / 靖 (如：玉堂贊化壇)", meaning: "法師個人的私密法場。壇為公有身份，靖為修行私室。上奏疏文時必須署名壇靖。" }
    ]
  },
  {
    category: "能量加持 (元神隨從)",
    id: "spirit",
    items: [
      { term: "元神月將", meaning: "根據受職月份分配的十二位月將元神。祂們負責在特定的時令下對法師進行能量加持。" },
      { term: "值日功曹使者", meaning: "法師行法時最親密的『公文傳遞員』。負責將燒化的表文以最快速度傳遞至天廷。" }
    ]
  }
];
...
export default function App() {
  ...
  const handleAction = () => {
    ...
    setTimeout(() => {
      try {
        ...
        const marshalIdx = getMarshalIdxByOffset(off);
        const m = MARSHAL_CANON[marshalIdx];
        const hourMsg = HOUR_MESSENGERS[form.hour];

        if (!jiazi || !m || !hourMsg) {
          throw new Error('映射資料缺失');
        }
        ...
        console.error(err);
        setError('無法產生結果，請確認輸入是否完整或嘗試再次送出。');
        setTab('home');
      } finally {
        setIsSpinning(false);
      }
  ...
  const handleSavePDF = () => {
    ...
    const opt = {
        ...
        html2canvas: { scale: 2, useCORS: true, backgroundColor: '#050505', scrollY: 0, width: 800, windowWidth: 800 },
  ...
  return (
    <div className="flex flex-col min-h-screen bg-[#010101] text-[#e5d5b0] font-serif overflow-x-hidden selection:bg-amber-900/40">
        ...
        <div className="fixed inset-0 z-[100] flex">
            ...
            <div className="relative w-64 bg-[#0a0a0a] border-r border-amber-900/40 h-full p-8 flex flex-col shadow-2xl font-sans animate-in slide-in-from-left duration-300 text-left">
                ...
                <nav className="space-y-4 flex-1">
                    ...
                    <button onClick={() => {setTab('explanation'); setMenuOpen(false);}} className={`w-full flex items-center gap-3 p-4 rounded-xl border transition-all ${tab === 'explanation' ? 'bg-amber-950/40 border-amber-500 text-amber-100' : 'border-zinc-900 text-zinc-400'}`}><HelpCircle size={20}/> <span className="font-bold">法語辭典</span></button>
                </nav>
            </div>
        </div>
      )}

      <main className="max-w-md mx-auto px-4 py-6 flex-1 w-full pb-24 font-sans text-center">
        {tab === 'home' && (
            ...
            <div className="bg-[#0a0a0a] rounded-2xl p-6 border border-zinc-900 shadow-2xl relative overflow-hidden text-zinc-300 text-left">
                <div className="flex justify-between items-center mb-6 border-b border-zinc-900 pb-4 font-black text-xs text-center">
                    ...
                    <div className="flex bg-black p-0.5 rounded-lg border border-zinc-800">
                        {['solar', 'lunar'].map(m => (
                            <button key={m} onClick={() => setForm({...form, calendar: m})} className={`px-4 py-1.5 rounded-md transition-all text-[10px] font-black ${form.calendar === m ? 'bg-amber-900 text-white shadow-lg' : 'text-zinc-700'}`}>{m === 'solar' ? '國曆' : '農曆'}</button>
                        ))}
                    </div>
                </div>
                <div className="space-y-6 text-left">
                    ...
                    <div className="grid grid-cols-4 gap-2 font-black text-[11px]">
                        {Object.keys(HOUR_MESSENGERS).map(s => (
                            <button key={s} onClick={() => setForm({...form, hour: s})} className={`py-3 rounded-xl border transition-all ${form.hour === s ? 'bg-amber-600 text-black border-amber-400 scale-105' : 'bg-black/40 border-zinc-900 text-zinc-700'}`}>{s}</button>
                        ))}
                    </div>
                </div>
                {error && (
                  <div className="mt-6 p-4 rounded-xl border border-amber-900/50 bg-amber-950/30 text-amber-300 text-sm font-bold shadow-inner">
                    {error}
                  </div>
                )}
        ...
        {/* 精簡版辭典 */}
        {tab === 'explanation' && (
          <div className="space-y-8 animate-in slide-in-from-right duration-500 font-sans pb-20 px-2 text-left">
            <div className="flex items-center gap-3 border-l-4 border-amber-600 pl-4 mb-8">
              <h2 className="text-xl font-black text-amber-100 tracking-widest italic uppercase">法語核心辭典</h2>
            </div>
            {DICTIONARY_DATA.map((group, idx) => (
              <div key={idx} className="space-y-4">
                <h3 className="text-amber-600 font-black text-sm tracking-[0.2em] uppercase opacity-80 pl-2 flex items-center gap-2 font-sans">
                  {group.category}
                </h3>
        ...
        {/* 結果頁面 (證書與 PDF) */}
        {tab === 'result' && !result && (
          <div className="text-center text-amber-200 py-12 font-sans">
            尚未產生結果，請回到錄籍頁面重新送出資訊。
            <div className="mt-4">
              <button onClick={() => setTab('home')} className="px-4 py-2 rounded-lg bg-amber-700 text-black font-black shadow-lg active:scale-95">
                回到錄籍
              </button>
            </div>
          </div>
          ...
          <div className="animate-in zoom-in-95 duration-700 pb-20 font-serif text-center text-zinc-300">
              <div className="mb-10 no-print space-y-4 text-left px-1">
                  <div className="flex justify-between items-center px-1">
                    ...
                    <button onClick={() => setTab('explanation')} className="text-xs text-amber-600 underline font-sans font-black flex items-center gap-1 font-sans"><HelpCircle size={14}/>解碼辭典</button>
                  </div>
              ...
              <div id="ordination-certificate" className="ordination-paper rounded-2xl p-0.5 shadow-2xl font-serif">
                  <div className="bg-[#050505] p-8 relative rounded-2xl border border-amber-900/10">
                      <div className="text-center mb-10 border-b border-amber-900/20 pb-8">
                          ...
                          <h2 className="font-black text-amber-100 tracking-[0.3em] drop-shadow-2xl text-3xl italic uppercase font-serif">職官分發告牒</h2>
                      </div>
                      <div className="space-y-12 leading-relaxed px-2 font-black text-left">
                          <div className="space-y-10 font-serif">
                              ...
                              <div className="flex flex-col space-y-2">
                                  <span className="text-label font-sans italic opacity-60 font-sans">一 補 充</span>
                                  <div className="pl-4 border-l-2 border-amber-900/40 py-1 text-content-row">
                                      <span className="block dynamic-gold font-bold">{result.office}</span>
                                      <div className="text-sm italic text-zinc-500 mt-1 font-sans">兼領與受領職權：</div>
                              ...
                              <div className="flex flex-col space-y-2">
                                  <span className="text-label font-sans italic opacity-60 font-sans">一 奏 立</span>
                                  <div className="pl-4 border-l-2 border-amber-900/40 py-1 text-content-row">
                                      <span><span className="dynamic-gold font-bold">{result.al}</span> <span className="dynamic-gold font-bold">{result.jn}</span></span>
                                  </div>
                              </div>
                              <div className="flex flex-col space-y-2">
                                  <span className="text-label font-sans italic opacity-60 uppercase tracking-widest font-black text-[10px] font-sans">一 泰玄都省正一平炁宮</span>
                                  <div className="pl-4 border-l-2 border-amber-900/40 py-1 text-content-row font-sans">
                                      ...
                                      <div className="text-sm text-zinc-500 mt-1 italic uppercase">隨身功曹：<span className="dynamic-gold font-bold">{result.hourMsg}</span></div>
                                  </div>
                              </div>
                              <div className="bg-[#0a0a0a] p-6 rounded-2xl border border-zinc-900 shadow-inner">
                                  <div className="flex flex-col space-y-5 text-left">
                                      <div className="pl-4 border-l-2 border-amber-900/20 py-1 text-content-row text-zinc-300">
                                          <span className="italic font-serif"><span className="dynamic-gold font-bold font-serif">{result.hl.mountain}{result.hl.emperor}{result.hl.qi}</span> 炁真人</span>
                                      </div>
                                      <div className="pl-4 border-l-2 border-amber-900/20 py-1 text-content-row text-zinc-300 font-serif">
                                          <span className="italic font-serif"><span className="dynamic-gold font-bold font-serif">{result.hl.qi}</span> 炁君 <span className="dynamic-gold font-bold font-serif">{result.hl.planet}</span> 三五步罡</span>
                                          <span className="block italic mt-1 font-bold font-sans font-serif">元命應 <span className="dynamic-gold font-bold font-serif">{result.hl.master}</span> 先生</span>
                                          <div className="text-sm text-zinc-500 mt-1 italic font-sans font-serif uppercase">元神：<span className="dynamic-gold font-bold font-serif">{result.monSpirit}</span>月將加持</div>
                                      </div>
                                      <div className="pl-4 border-l-2 border-amber-900/20 py-1 text-content-row text-zinc-400 font-serif">
                                          <span className="italic">配屬本命北斗 <span className="dynamic-gold font-bold font-serif">{result.st}</span> 星君</span>
                                      </div>
                                  </div>
                              </div>
                              <div className="flex flex-col space-y-4 pt-10 border-t border-zinc-900/50 text-left font-serif">
                                  ...
                                  <div className="flex flex-col pl-4 py-2 border-l-2 border-amber-900/40 text-content-row">
                                      <span className="text-zinc-300 font-bold uppercase font-serif"><span className="dynamic-gold font-bold">{result.prefix}{result.marshal}</span> 麾下</span>
                                  </div>
                              </div>
                          </div>
                      </div>
        ...
        {/* 元帥全錄 */}
        {tab === 'catalog' && (
          <div className="animate-in slide-in-from-right duration-500 pb-40 space-y-6 font-sans text-left px-2 text-center text-zinc-300">
              ...
              <div className="relative group font-sans font-bold">
              ...
              <div className="bg-[#0a0a0a] rounded-2xl border border-zinc-900 p-4 shadow-2xl font-black text-center font-serif">
                  <div className="grid grid-cols-2 gap-3 h-[70vh] overflow-y-auto custom-scrollbar px-1 text-xs text-zinc-300">
                      {JIAZI_60.map((name, index) => {
                          ...
                          const m = MARSHAL_CANON[getMarshalIdxByOffset(index)];
                          ...
                          return (
                              <div key={name} className={`bg-zinc-950/40 p-5 rounded-3xl border border-zinc-900/60 flex flex-col items-center justify-center space-y-2 transition-all ${isGroup ? 'border-l-2 border-l-amber-600/40' : ''}`}>
                                  <div className="flex justify-between w-full px-1 items-center opacity-60 font-sans font-serif"><span className="text-zinc-800 font-sans font-black">#{m.rank}</span><span className="text-zinc-500 font-bold font-sans">{cycleYear}</span></div>
                                  <span className="text-zinc-400 font-black text-sm font-sans font-serif">{name}</span>
                                  <span className="block font-black text-amber-600 italic tracking-tighter uppercase font-serif text-lg font-serif">{m.name}</span>
                              </div>
                          );
                      })}
                  </div>
      ...
      <nav className="fixed bottom-0 left-0 right-0 bg-black/95 backdrop-blur-3xl border-t border-amber-900/20 p-4 pb-8 z-50 max-w-md mx-auto rounded-t-[2.5rem] shadow-2xl no-print font-sans font-black text-[11px]">
          <div className="flex justify-around items-center px-4 font-sans">
              <button onClick={() => setTab('home')} className={`flex flex-col items-center space-y-1 px-4 py-2 rounded-2xl transition-all relative ${tab === 'home' || tab === 'result' ? 'text-amber-400 scale-110' : 'text-zinc-700 opacity-60'}`}><Scroll size={20}/><span className="uppercase tracking-widest font-bold">登記</span></button>
              <button onClick={() => setTab('explanation')} className={`flex flex-col items-center space-y-1 px-4 py-2 rounded-2xl transition-all relative ${tab === 'explanation' ? 'text-amber-400 scale-110' : 'text-zinc-700 opacity-60'}`}><HelpCircle size={20}/><span className="uppercase tracking-widest font-bold">詞典</span></button>
              <button onClick={() => setTab('catalog')} className={`flex flex-col items-center space-y-1 px-4 py-2 rounded-2xl transition-all relative ${tab === 'catalog' ? 'text-amber-400 scale-110' : 'text-zinc-700 opacity-60'}`}><BookOpen size={20}/><span className="uppercase tracking-widest font-bold">元帥</span></button>
          </div>
      </nav>
    </div>
  );
