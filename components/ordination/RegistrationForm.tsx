...
const RegistrationForm:React.FC<{manager:OrdinationManager}>=({manager:m})=><aside className="control-panel">
  ...
  <section className="form-section"><label className="field-label" htmlFor="roc-year">出生年</label><div className="year-input-wrap"><span>民國</span><input id="roc-year" className="text-input year-input" type="number" min="1" max="200" inputMode="numeric" value={m.rocYear} onChange={e=>m.setRocYear(Math.max(1,Math.min(200,Number(e.target.value)||1)))}/><span>年</span></div><p className="field-help">民國 {m.rocYear} 年＝西元 {m.rocYear+1911} 年</p></section>
  <section className="form-section"><label className="field-label">農曆月日</label><div className="date-grid two-date"><label><span>月份</span><select className="select" value={m.lunarMonth} onChange={e=>m.setLunarMonth(Number(e.target.value))}>{lunarMonths.map((name,i)=><option key={name} value={i+1}>{name}</option>)}</select></label><label><span>日期</span><select className="select" value={m.lunarDay} onChange={e=>m.setLunarDay(Number(e.target.value))}>{Array.from({length:m.monthDays},(_,i)=>i+1).map(v=><option key={v} value={v}>{lunarDays[v-1]}</option>)}</select></label></div>{m.lunarYearInfo.leapMonth===m.lunarMonth&&<label className="leap-toggle"><input type="checkbox" checked={m.isLeapMonth} onChange={e=>m.setIsLeapMonth(e.target.checked)}/><span>這一天是閏{lunarMonths[m.lunarMonth-1]}</span></label>}</section>
  ...
  <button className="primary-action" onClick={m.generate}>換算八字並查考玉格<Icon name="arrow"/></button>{m.result&&<button className="secondary-action" onClick={m.saveDisciple}>儲存至本機清冊</button>}<p className="privacy-note">資料只儲存在此裝置的瀏覽器</p>
</aside>;
