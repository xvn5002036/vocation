...
import { Shield, Sparkles, Scroll, User, MapPin, Compass, Flame, Zap, Camera, RotateCcw, Info } from 'lucide-react';
...
const InputSection = ({ formData, setFormData, handleStart }) => (
  <div className="max-w-xl mx-auto bg-slate-800/50 p-6 rounded-2xl border border-slate-700 backdrop-blur-sm shadow-xl">
    <div className="text-center mb-6">
      ...
      <p className="text-slate-400 text-xs mt-1">依據《天壇玉格》法統 · 干支精確演算</p>
    </div>
    <div className="space-y-4">
      <div>
        <label className="block text-yellow-500/80 text-xs mb-1 ml-1 font-medium">道友法名 / 姓名</label>
        <input className="w-full bg-slate-900 border border-slate-700 rounded-xl px-4 py-3 text-white focus:outline-none focus:ring-2 focus:ring-yellow-500/50 text-sm" placeholder="請輸入姓名" value={formData.name} onChange={(e) => setFormData({...formData, name: e.target.value})} />
      </div>
      ...
      <div className="grid grid-cols-2 gap-4">
        <div>
          <label className="block text-yellow-500/80 text-xs mb-1 ml-1 font-medium">農曆年 (西元數)</label>
...
const App = () => {
  ...
  const handleStart = () => {
    ...
    // 4. 元帥分配邏輯 (不再鎖定，而是根據時辰與天干交叉計算)
    const hourIdx = LUNAR_HOURS.indexOf(formData.birthHour); 
    const nameScore = formData.name.length; 
    const combinedSeed = (hourIdx + nameScore) % 3; // 使用時辰與姓名長度作為偏移灵數

    let mId = formData.selectedMarshalId;
    if (!mId) {
      const marshalPool = STEM_MARSHALS[gz.stem] || STEM_MARSHALS["甲"]; // 依天干選部隊
      // 在部隊池中根據偏移靈數選取，確保同部隊不重複
      mId = marshalPool[(roleIdx + combinedSeed) % marshalPool.length];
    }
    ...
    setResult({ gz, zhiInfo, rankData, appoint, office, marshalInfo, variantDesc: mTitleData?.desc, starInfo, correlation: STEM_CORRELATION[gz.stem] });
    setActiveTab('decree');
  };
  ...
  return (
    <div className="min-h-screen bg-slate-950 text-slate-100 font-serif p-4">
      <header className="text-center mb-8">
        <h1 className="text-2xl md:text-4xl font-bold text-transparent bg-clip-text bg-gradient-to-b from-yellow-200 to-yellow-600">※ 正一法壇 · 職官分發 ※</h1>
        <div className="h-1 w-24 bg-yellow-600 mx-auto rounded-full mt-2" />
      </header>
      {!result ? ( <InputSection formData={formData} setFormData={setFormData} handleStart={handleStart} /> ) : (
        <div className="max-w-4xl mx-auto space-y-6">
          ...
          <div ref={cardRef} className="bg-slate-900 border-4 border-yellow-600/50 p-6 md:p-10 rounded shadow-2xl relative overflow-hidden">
            {activeTab === 'decree' ? (
              <div className="space-y-6 text-center md:text-left">
                <div className="text-center border-b border-yellow-600/30 pb-4"><h2 className="text-2xl font-bold text-yellow-500 tracking-widest">錄壇奏職寶誥</h2></div>
                <div className="grid md:grid-cols-2 gap-6 text-sm md:text-base">
                  ...
                  <div className="p-4 bg-yellow-600/10 rounded border border-yellow-600/20 flex flex-col justify-center">
                ...
                <p className="text-xs italic text-slate-400 text-center mt-4">「依據{result.correlation.direction}方{result.correlation.color}帝{result.correlation.qi}炁之法力，授以此籙，統御鬼神。」</p>
              </div>
            ) : (
              <div className="space-y-4">
                <div className={`flex items-center gap-4 ${result.marshalInfo.color} p-4 rounded-xl text-white shadow-lg`}>
                  <Shield className="w-12 h-12" />
                  <div><h3 className="text-2xl font-bold">{result.marshalInfo.name}</h3><p className="text-xs opacity-80">職銜：{result.variantDesc}</p></div>
                </div>
                <div className="bg-slate-800/50 p-4 rounded-xl border border-slate-700 text-xs md:text-sm leading-relaxed text-slate-300 whitespace-pre-wrap">{result.marshalInfo.intro}</div>
              </div>
            )}
          </div>
          <div className="flex justify-center gap-4 pb-8"><button onClick={exportImage} className="px-8 py-3 bg-emerald-700 text-white rounded-xl shadow-lg flex items-center gap-2"><Camera className="w-5 h-5" /> 保存法職寶誥</button><button onClick={() => setResult(null)} className="px-8 py-3 bg-slate-800 text-slate-300 rounded-xl">重新選取</button></div>
        </div>
      )}
    </div>
  );
