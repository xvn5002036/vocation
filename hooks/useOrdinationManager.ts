...
export function useOrdinationManager(){
  ...
  const openRecord=(record:PersonnelRecord)=>{setRocYear(record.rocYear);setLunarMonth(record.lunarMonth);setLunarDay(record.lunarDay);setIsLeapMonth(record.isLeapMonth);setHourBranch(record.hourBranch);setDiscipleName(record.name);setResult(calculateJadeRegistry(record));setView('generate');};
